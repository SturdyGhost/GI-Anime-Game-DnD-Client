...
func _on_weapon_button_pressed() -> void:
	...
	var detail_scene = preload("res://Scene's/weapon_detail_scene.tscn").instantiate()
	# Make sure the root node is not using anchors or layout presets
	detail_scene.anchor_left = 0
	detail_scene.anchor_top = 0
	detail_scene.anchor_right = 0
	detail_scene.anchor_bottom = 0
	
	detail_scene.offset_left = 0
	detail_scene.offset_top = 0
	detail_scene.offset_right = 0
	detail_scene.offset_bottom = 0
	detail_scene.custom_minimum_size = Vector2(3000, 1600)  # Adjust as needed
